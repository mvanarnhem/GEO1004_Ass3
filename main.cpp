...
void write_to_json(std::map<int, std::vector<std::vector<Point_3>>> &room_faces, Point_3 &origin);
...
int main() {
    ...
    write_to_json(surfaces_assigned, origin);
    return 0;
}

void write_to_json(std::map<int, std::vector<std::vector<Point_3>>> &room_faces_map, Point_3 &origin) {
    json json;
    json["type"] = "CityJSON";
    json["version"] = "2.0";
...
//    json["CityObjects"]["Building"]["vertices"] = json::array();


    // extract vertices from map to write to json
    unsigned int vertex_counter = 0;
    std::vector<Point_3> all_vertices_found;

    for (const auto &room_pair: room_faces_map) {
        for (const auto &face: room_pair.second) {
            for (const auto &vertex: face) {
                all_vertices_found.push_back(vertex);
            }
        }
    }

//    for (auto vertex : all_vertices_found) {
//        // if close to existing vertex
//
//        if (! unique_vertices.empty()) {
//            for (auto pair : unique_vertices) {
//                if (std::abs(pair.first.x() - vertex.x()) < 0.0001 &&
//                    std::abs(pair.first.y() - vertex.y()) < 0.0001 &&
//                    std::abs(pair.first.z() - vertex.z()) < 0.0001) {
//                    std::cout << "vertex too close to add as unique" << std::endl;
//                } else {
//                    unique_vertices[vertex] = vertex_counter;
//                    vertex_counter++;
//                    std::cout << "unique vertices added: "<< vertex_counter << std::endl;
//                }
//            }
//
//
...
//    }
    ...
    std::map<Point_3, int> unique_vertices_map;

    float threshold = 0.0001;

    for (const auto &vertex : all_vertices_found) {
        if (unique_vertices.empty()) {
            unique_vertices.push_back(vertex);
            std::cout << "no unique vertices, appended the first" << std::endl;
        } else {
            for (const auto &uni_vertex : unique_vertices) {
                double distance = CGAL::squared_distance(vertex, uni_vertex);
                if (distance > threshold) {
                    unique_vertices.push_back(vertex);
                    std::cout << "vertex far enough to be added as unique" << std::endl;
                } else {
                    std::cout << "vertex too close!; not added" << std::endl;
                }
            }
        }
    }
    std::cout << "total vertices found:\t\t" << all_vertices_found.size()  << std::endl;
    std::cout << "of which are unique:\t\t" << unique_vertices.size()  << std::endl;

    for (auto &vertex : all_vertices_found) {
        json["vertices"].push_back({vertex.x(), vertex.y(), vertex.z()});
    }
    ...
    if (out_stream.is_open()) {
        ...
        out_stream.close();
        std::cout << "CityJSON file generated: mybuilding.json" << std::endl;
    } else {
